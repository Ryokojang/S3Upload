...
public class Reporter : MonoBehaviour
{
    ...
    bool isClickGesture()//##ClickGesture
    {
        if (Application.platform == RuntimePlatform.Android ||
            Application.platform == RuntimePlatform.IPhonePlayer)
        {
        ...
        if (StartTime != 0)
        {
            ...
            if (NowTime - StartTime > timeToShow)
            {
                StartTime = 0;
                return true;
            }
	...
	void Update()
	{
		//--- 최적화를 위해 show == true 일때만 실행되도록 설정 --- 한진
		if (show)
        {
        ...
        if (enableLogViewer && isClickGesture())
        {
            if (!show)
            {
                doShow();
            }
            /*else//##ClickGesture : 클릭제스쳐로 보이게만 함, 보려고 하면 자꾸 꺼져서 이렇게 처리함 
            {
                StartTime = 0;
